...
rank = 'Species'
X, rank, splits, class_list, _ = hierarchy_filter(data, rank = 'Species', min_seq = 5, wildcard_seed = False, wildcard_list = None,
...
for conv_type in ['GraphConv', 'gin', 'sage', 'gcn']:
    ...
    for split_num, (i, j) in enumerate(splits.split(X.smiles.values.tolist(), X[rank].values.tolist())):
        ...
        train_data_list, train_classes = from_smiles_list(train_x, train_y)
        test_data_list, test_classes = from_smiles_list(test_x, test_y)
        ...
        num_classes = len(train_classes)  # Number of classes for classification, use 1 for regression
        print('before entering into model', num_classes)
        ...
        if performance_metric['accuracy'] > best_overall_acc:
            ...
            model_path = Path('models')
            model_path.mkdir(parents = True, exist_ok=True)
            model_name = f'best_{conv_type}_{rank}_model.pth'
            model_save_path = model_path / model_name
            model.load_state_dict(best_model_wts)          
            torch.save(best_model_wts, model_save_path)
...
# print(lib_size)
rank = 'Species'
X, rank, splits, class_list, _ = hierarchy_filter(data, rank = 'Species', min_seq = 5, 
                                                                wildcard_seed = False, 
                                                                wildcard_list = None,
                                                                wildcard_name = None, r = 0.1)
print(X.shape)
...
for split_num, (i,j) in enumerate(splits.split(X.target.values.tolist(), X[rank].values.tolist())):
   ...
   if best_acc > best_overall_acc:
    ...
    model_path = Path('models')
    model_path.mkdir(parents = True, exist_ok=True)
    model_name = f'best_{rank}_model.pth'
    model_save_path = model_path / model_name
    model.load_state_dict(best_model_wts)          
    torch.save(best_model_wts, model_save_path)
