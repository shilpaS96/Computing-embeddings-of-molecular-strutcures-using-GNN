...
rank = 'Phylum'
X, rank, splits, class_list = hierarchy_filter(data, rank = 'Phylum', min_seq = 5, wildcard_seed = False, wildcard_list = None,
...
for conv_type in ['GraphConv', 'gin', 'sage', 'gcn']:
    ...
    for split_num, (i, j) in enumerate(splits.split(X.smiles.values.tolist(), X[rank].values.tolist())):
        ...
        model_ft, performance_metric, best_model_wts, test_losses, test_acc, epoch = train_model(model, criterion, 
        ...
        if performance_metric['accuracy'] > best_overall_acc:
            ...
            output_path = f'./plots/output_{rank}_{conv_type}_plot.png' 
