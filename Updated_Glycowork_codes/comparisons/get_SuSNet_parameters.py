...
#rank = 'Class'
ranks = ['Phylum', 'Kingdom', 'Domain', 'immunogenicity', 'Class' , 'Order', 'Family', 'Genus', 'Species']
parameters = []
data_dist = []
for rank in ranks:
    X, rank, splits, class_list = hierarchy_filter(data, rank = rank, min_seq = 5, wildcard_seed = False, wildcard_list = None,
    ...
    data_dist.append((rank, X.shape, len(class_list)))
    ...
    for conv_type in ['GraphConv', 'gin', 'sage', 'gcn']:
        ...
        for split_num, (i, j) in enumerate(splits.split(X.smiles.values.tolist(), X[rank].values.tolist())):
            ...
            parameters.append((rank, conv_type, num_params))
            break
print(parameters)
print(data_dist)
