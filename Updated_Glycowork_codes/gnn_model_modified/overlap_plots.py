...
print(data.shape)
merged_df = pd.merge(X, data[['smiles', 'predicted_taxonomy']], on='smiles', how='left')


print(merged_df)
print(merged_df.shape)
#merged_df = merged_df[merged_df['predicted_taxonomy'] != ' ']
merged_df = merged_df.dropna(subset=['predicted_taxonomy'])
print(set(merged_df.predicted_taxonomy.values.tolist()))
print('data after removing nan: ', merged_df)
print(merged_df.shape)
...
for conv_type in ['GraphConv']: #, 'gin', 'sage', 'gcn']:
    ...
    for split_num, (i, j) in enumerate(splits.split(merged_df.smiles.values.tolist(), merged_df[rank].values.tolist())):
        print(f"split {split_num} is taking place")
        print('^-^' * (split_num+1))
        #print(i, j)
        #print(df.smiles.values.tolist())
        
        train_x = [merged_df.smiles.values.tolist()[k] for k in i]
        train_y = [merged_df[rank].values.tolist()[k] for k in i]
        #print(train_x)
        print(len(train_x))
        len_train_x = [len(k) for k in train_x]
        #print((len_train_x))
        test_x = [merged_df.smiles.values.tolist()[k] for k in j]
        test_y = [merged_df[rank].values.tolist()[k] for k in j]
        #print(test_x)
        #print(len(test_x))
        ...
        if performance_metric['accuracy'] > best_overall_acc:
            ...
            ## save plots
            val_accuracy['SuSNet'] = test_acc
            val_error['SuSNet'] = test_losses
            mcc['SuSNet'] = test_mcc
            val_epoch['SuSNet'] = range(epoch+1)
...
def train_model(model, criterion, optimizer, scheduler, num_epochs = 25):
  ...
  plt.xlabel('Number of Epochs')
  plt.legend(['Validation Accuracy'], loc = 'best')
  return model, best_model_wts, best_acc, best_loss, val_losses, val_acc, val_mcc, epoch
...
# print(lib_size)
rank = 'Domain'
X, rank, splits, class_list, _ = hierarchy_filter(data, rank = 'Domain', min_seq = 5, 
                                                                wildcard_seed = False, 
                                                                wildcard_list = None,
                                                                wildcard_name = None, r = 0.1)
print(X.shape)
...
for split_num, (i,j) in enumerate(splits.split(X.target.values.tolist(), X[rank].values.tolist())):
   ...
   glycan_graphs_train, _ = dataset_to_graphs(train_x, train_y, libr = lib, error_catch = True)
   glycan_graphs_val, _ = dataset_to_graphs(val_x, val_y, libr = lib)


   train_loader = DataLoader(glycan_graphs_train, batch_size = 32, shuffle = True, drop_last=True)
   val_loader = DataLoader(glycan_graphs_val, batch_size = 32, shuffle = False, drop_last=True)
   ...
   scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_ft, 50)
   criterion = torch.nn.CrossEntropyLoss().to(device)
   model_ft, best_model_wts, best_acc, best_loss, val_losses, val_acc, val_mcc, epoch  = train_model(model, criterion, optimizer_ft, scheduler,
                    num_epochs = 100)
   ...
   if best_acc > best_overall_acc:
    ...
    val_accuracy['sweetnet'] = val_acc
    val_error['sweetnet'] = val_losses
    mcc['sweetnet'] = val_mcc
    val_epoch['sweetnet'] = range(epoch+1)
    
    """
    fig, ax = plt.subplots(nrows = 1, ncols = 1) 
    plt.subplot(1, 1, 1)
    plt.plot(range(epoch+1), val_mcc)
    plt.title('Training of SweetNet')
    plt.ylabel('Validation MCC')
    plt.xlabel('Number of Epochs')
    plt.legend(['MCC score'],loc = 'best')
    ...
    plot_path = Path('mcc_plots')
    plot_path.mkdir(parents = True, exist_ok=True)
    output_path = f'./mcc_plots/output_{rank}_plot.png' 
...
print(f"Accuracy: {mean_accuracy:.4f} ± {std_accuracy:.4f}")
result_list.append({"Model":'SweetNet',
                        'Classification': rank,
                        'Accuracy': accuracy_str})        
...
plt.plot(val_epoch['SuSNet'], val_accuracy['SuSNet'], label="SuperSweetNet Accuracy",  color='b')
plt.plot(val_epoch['sweetnet'], val_accuracy['sweetnet'], label="SweetNet Accuracy",  color='r')
...
plot_path = Path('revised_plots')
plot_path.mkdir(parents = True, exist_ok=True)
output_path = f'./revised_plots/acc_output_{rank}_plot.png' 
...
plt.figure(figsize=(10, 6))
plt.plot(val_epoch['SuSNet'], val_error['SuSNet'], label="SuperSweetNet Accuracy", color='b')
plt.plot(val_epoch['sweetnet'], val_error['sweetnet'], label="SweetNet Accuracy",  color='r')
...
plot_path = Path('revised_plots')
plot_path.mkdir(parents = True, exist_ok=True)
output_path = f'./revised_plots/loss_output_{rank}_plot.png' 
...
plt.figure(figsize=(10, 6))
plt.plot(val_epoch['SuSNet'], mcc['SuSNet'], label="SuperSweetNet Accuracy",  color='b')
plt.plot(val_epoch['sweetnet'], mcc['sweetnet'], label="SweetNet Accuracy",  color='r')

# Labels and Title
plt.xlabel("Epochs")
plt.ylabel("Validation MCC")
plt.title("Model MCC score Comparison")
plt.legend()

plt.tight_layout()

plot_path = Path('revised_mcc_plots')
plot_path.mkdir(parents = True, exist_ok=True)
output_path = f'./revised_mcc_plots/output_{rank}_plot.png' 
