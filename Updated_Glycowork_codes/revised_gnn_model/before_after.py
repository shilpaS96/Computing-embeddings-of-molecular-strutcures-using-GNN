...
rank = 'Kingdom'
X, rank, splits, class_list, _ = hierarchy_filter(data, rank = 'Kingdom', min_seq = 5, wildcard_seed = False, wildcard_list = None,
...
for conv_type in ['GraphConv']: #, 'gin', 'sage', 'gcn']:
    ...
    for split_num, (i, j) in enumerate(splits.split(X.smiles.values.tolist(), X[rank].values.tolist())):
        ...
        if performance_metric['accuracy'] > best_overall_acc:
            ...
            model_path = Path('models')
            model_path.mkdir(parents = True, exist_ok=True)
            model_name = f'before_best_{conv_type}_model.pth'
            model_save_path = model_path / model_name
            model.load_state_dict(best_model_wts)          
            torch.save(best_model_wts, model_save_path)
            
            ## save plots
            val_accuracy['before'] = test_acc
            val_error['before'] = test_losses
            mcc['before'] = test_mcc
            val_epoch['before'] = range(epoch+1)
...
for conv_type in ['GraphConv']: #, 'gin', 'sage', 'gcn']:
    ...
    for split_num, (i, j) in enumerate(splits.split(X.smiles.values.tolist(), X[rank].values.tolist())):
        ...
        if performance_metric['accuracy'] > best_overall_acc:
            ...
            model_path = Path('models')
            model_path.mkdir(parents = True, exist_ok=True)
            model_name = f'after_best_{conv_type}_model.pth'
            model_save_path = model_path / model_name
            model.load_state_dict(best_model_wts)          
            torch.save(best_model_wts, model_save_path)
            
            ## save plots
            val_accuracy['after'] = test_acc
            val_error['after'] = test_losses
            mcc['after'] = test_mcc
            val_epoch['after'] = range(epoch+1)
...
plt.plot(val_epoch['before'], val_accuracy['before'], label="SuperSweetNet Accuracy",  color='b')
plt.plot(val_epoch['after'], val_accuracy['after'], label="SweetNet Accuracy",  color='r')
...
plot_path = Path('revised_plots_beaf')
plot_path.mkdir(parents = True, exist_ok=True)
output_path = f'./revised_plots_beaf/acc_output_{rank}_plot.png' 
...
plt.figure(figsize=(10, 6))
plt.plot(val_epoch['before'], val_error['before'], label="SuperSweetNet Loss", color='b')
plt.plot(val_epoch['after'], val_error['after'], label="SweetNet Loss",  color='r')
...
plot_path = Path('revised_plots_beaf')
plot_path.mkdir(parents = True, exist_ok=True)
output_path = f'./revised_plots_beaf/loss_output_{rank}_plot.png' 
...
plt.figure(figsize=(10, 6))
plt.plot(val_epoch['before'], mcc['before'], label="SuperSweetNet MCC",  color='b')
plt.plot(val_epoch['after'], mcc['after'], label="SweetNet MCC",  color='r')
...
plot_path = Path('revised_plots_beaf')
plot_path.mkdir(parents = True, exist_ok=True)
output_path = f'./revised_plots_beaf/output_{rank}_plot.png' 
