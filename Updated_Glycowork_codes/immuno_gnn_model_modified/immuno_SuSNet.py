...
def train_model(model, criterion, optimizer, scheduler, dataloaders, conv_type, num_epochs = 30):
    ...
    plt.show()
    return model, best_acc, best_loss, best_model_wts
...
rank = 'immunogenicity'
X, rank, splits, class_list = hierarchy_filter(data, rank = 'immunogenicity', min_seq = 5, wildcard_seed = False, wildcard_list = None,
...
for conv_type in ['GraphConv', 'gin', 'sage', 'gcn']:
    ...
    for split_num, (i, j) in enumerate(splits.split(X.smiles.values.tolist(), X[rank].values.tolist())):
        ...
        model_ft, best_acc, best_loss, best_model_wts = train_model(model, criterion, optimizer_ft, scheduler, dataloaders,
