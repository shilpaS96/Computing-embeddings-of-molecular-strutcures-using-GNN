...
X_out, _ , _ , class_list, class_converter = hierarchy_filter(data, rank = 'Domain', min_seq = 5, wildcard_seed = False, wildcard_list = None,
...
for k, v in embeddings.items():
    ...
    sns.scatterplot(x = tsne_emb[:,0], y = tsne_emb[:,1], s = 20, alpha = 0.4,
                hue = data.predicted_taxonomy.values.tolist, palette = 'colorblind', rasterized = True, legend=False)
    ...
    output_path = f'./revised_embeddings/output_{rank}_{k}_plot.png' 

    plt.savefig(output_path)
    print("Plot saved as 'output_{rank}_{k}_plot.png'")
