      ...
      if wo_labels:
        ...
        for k in glycan_graphs:
          ...
          if len(k[1]) < 2:
                ...
                count += 1
                # print(f"k: {k}")
                # print(f"k[1]: {k[1]}, len(k[1]): {len(k[1]) if k[1] else 'None'}")
                # print(k[1][0],k[1][1])
                # print(f"Skipping entry with insufficient edge data: {k}")
                glycan_graphs.remove(k)
        # print(count)
        #print(len(glycan_graphs))
        #print(count)
